...
zathura_error_t jpg_document_open(zathura_document_t* document) {
  /* get path and password */
  const char* filename = zathura_document_get_path(document);

  cairo_surface_t* surface = cairo_image_surface_create_from_jpeg(filename);
...
zathura_error_t jpg_document_free(zathura_document_t* document, void* data) {
  if (document == NULL) {
